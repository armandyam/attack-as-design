...
S=np.load("gfp_surrogate.npz"); seeds=json.load(open("gfp_seeds.json"))
H=np.load("gfp_baseline_heads.npz")
BB=np.load("blosum62.npz"); BLO=BB["B"]; BORD="".join(BB["order"]); BIDX={a:i for i,a in enumerate(BORD)}
bands=np.load("gfp_biophys_bands.npz")
...
for si,sd in enumerate(seeds):
    ss=sd["seq"]; row={"idx":sd["idx"],"true_score":sd["true_score"],"len":sd["len"],"seed_seq":ss}
    for mth in METHODS:
...
os.makedirs("out",exist_ok=True); json.dump(results,open("out/gfp_headtohead.json","w"))
